...
def strip_html(text: str) -> str:
    """Remove HTML tags (e.g. <i>, </i>) from a string."""
...
def paper_row(p: dict) -> str:
    ...
    doi_lnk = (f'<a href="{p["doi_url"]}" style="color:#2563eb;">'
               f'{p["doi"][:40]}…</a>' if p.get("doi_url") else "—")
    score   = p.get("score", 0)
    score_color = "#059669" if score >= 60 else ("#d97706" if score >= 40 else "#64748b")
    title   = strip_html(p.get("title", ""))[:120]
    return f"""
        <tr>
          <td style="padding:10px 8px;border-bottom:1px solid #e2e8f0;vertical-align:top;width:60%;">
          ...
          <td style="padding:10px 8px;border-bottom:1px solid #e2e8f0;text-align:center;width:10%;">
            <span style="color:{score_color};font-weight:700;font-size:15px;">{score}</span>
          </td>
          <td style="padding:10px 8px;border-bottom:1px solid #e2e8f0;width:30%;font-size:11px;">
            {doi_lnk}
          </td>
        </tr>"""
        ...
        <table style="width:100%;border-collapse:collapse;">
          <thead>
            <tr style="background:#f1f5f9;">
              <th style="padding:8px;text-align:left;color:#475569;font-size:11px;font-weight:600;">TITLE</th>
              <th style="padding:8px;text-align:center;color:#475569;font-size:11px;font-weight:600;">SCORE</th>
              <th style="padding:8px;text-align:left;color:#475569;font-size:11px;font-weight:600;">DOI</th>
            </tr>
          </thead>
          <tbody>{rows}</tbody>
        </table>"""
...
<body style="margin:0;padding:0;background:#ffffff;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
  <div style="max-width:800px;margin:0 auto;padding:24px;background:#ffffff;color:#1e293b;">

    <!-- Header -->
    <div style="background:linear-gradient(135deg,#dbeafe,#e0f2fe);
      ...
      <div style="display:flex;align-items:center;gap:12px;">
        <div style="width:40px;height:40px;background:#2563eb;border-radius:8px;
                    ...
                    font-size:20px;">&#128270;</div>
        <div>
          <h1 style="color:#1e293b;margin:0;font-size:22px;font-weight:700;">Tech Vigilance</h1>
    ...
    <div style="margin-top:40px;padding:20px;border-top:1px solid #e2e8f0;text-align:center;">
      <a href="{PAGES_URL}" style="display:inline-block;background:#2563eb;color:#fff;
